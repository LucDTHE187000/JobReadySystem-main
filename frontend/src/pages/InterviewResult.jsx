...
export default function InterviewResult() {
    ...
    const [recPassed, setRecPassed] = useState(true);
    const [recCompleted, setRecCompleted] = useState(true);
    ...
    const fetchRecommendedJobs = async () => {
        try {
            setJobsLoading(true);
            const res = await axios.get(`${API_URL}/api/jobs/recommendations/${sessionId}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
            
            setRecCompleted(res.data.completed !== false);
            setRecPassed(res.data.passed !== false);
            ...
            setRecPassed(true);
            setRecMessage('Chúc mừng bạn đã hoàn thành buổi phỏng vấn! Dưới đây là danh sách việc làm gợi ý dành cho bạn.');
            
            // Fallback
            if (session?.jobTitle) {
                try {
                    const fallbackRes = await axios.get(`${API_URL}/api/jobs/search`, {
                        params: { keyword: session.jobTitle, limit: 3 }
                    });
                    setRecommendedJobs(fallbackRes.data.data || []);
                } catch (fallbackErr) {
            ...
            <div className="max-w-6xl mx-auto w-full space-y-6">
                ...
                <div className="bg-white/10 border border-white/10 backdrop-blur-md rounded-3xl shadow-xl overflow-hidden">
                    <div className="bg-gradient-to-r from-[#0A2463] to-[#071A4A] text-white px-8 py-8">
                        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                            <div>
                                <h1 className="text-3xl font-bold mb-2">Kết quả phỏng vấn - {session.jobTitle}</h1>
                            ...
                            <div className="flex items-center gap-6">
                                <div className="text-center">
                                    <p className="text-sm text-white/70 mb-1">Điểm đánh giá tổng quan</p>
                ...
                <div className="grid md:grid-cols-2 gap-6">
                    <div className="bg-white/10 border border-white/10 backdrop-blur-md rounded-3xl shadow-xl p-8">
                        ...
                        <div className="space-y-4">
                            {[
                                { label: 'Khả năng giao tiếp', score: skillScores.communication, icon: MessageSquare, color: 'bg-blue-500' },
                                { label: 'Kiến thức chuyên môn', score: skillScores.technical, icon: Brain, color: 'bg-purple-500' },
                                { label: 'Giải quyết vấn đề', score: skillScores.problemSolving, icon: Target, color: 'bg-green-500' },
                                { label: 'Sự tự tin', score: skillScores.confidence, icon: Award, color: 'bg-amber-500' },
                                { label: 'Phong thái chuyên nghiệp', score: skillScores.professionalism, icon: CheckCircle, color: 'bg-pink-500' }
                            ].map((skill, idx) => (
                                <div key={idx}>
                    ...
                    <div className="bg-white/10 border border-white/10 backdrop-blur-md rounded-3xl shadow-xl p-8">
                        <h2 className="text-2xl font-bold text-white mb-6 flex items-center gap-2">
                            ...
                            Thông số buổi phỏng vấn
                        </h2>
                        <div className="grid grid-cols-2 gap-4">
                            <div className="bg-white/5 border border-white/5 rounded-2xl p-4">
                                <p className="text-sm text-white/60 font-semibold mb-1">Thời gian</p>
                                <p className="text-2xl font-bold text-white">{Math.floor(session.duration / 60)} phút {session.duration % 60} giây</p>
                            </div>
                            <div className="bg-white/5 border border-white/5 rounded-2xl p-4">
                                <p className="text-sm text-white/60 font-semibold mb-1">Số câu hỏi</p>
                            ...
                            <div className="bg-white/5 border border-white/5 rounded-2xl p-4">
                                ...
                                <p className="text-2xl font-bold text-white">{session.interviewType === 'Mixed' ? 'Tổng hợp' : session.interviewType}</p>
                            </div>
                            <div className="bg-white/5 border border-white/5 rounded-2xl p-4">
                                <p className="text-sm text-white/60 font-semibold mb-1">Mức độ khó</p>
                ...
                <div className="bg-white/10 border border-white/10 backdrop-blur-md rounded-3xl shadow-xl overflow-hidden">
                    <div className="bg-white/5 px-8 py-6 border-b border-white/10">
                        <h3 className="text-xl font-bold text-white">Phân tích chi tiết từng câu hỏi</h3>
                    </div>
                                ...
                                <div key={q._id} className="border border-white/10 bg-white/5 rounded-2xl overflow-hidden">
                                    ...
                                    >
                                        <div className="text-left flex-1">
                                            <p className="text-sm font-semibold text-[#F5C518] mb-1">Câu hỏi {index + 1} ({q.questionType === 'Technical' ? 'Chuyên môn / Kỹ thuật' : 'Tình huống / Hành vi'})</p>
                                    ...
                                    {expandedQuestions[q._id] && (
                                        <div className="px-6 pb-6 space-y-4 bg-white/5">
                                            <div>
                                                <p className="text-xs font-bold text-[#F5C518] mb-2 uppercase">Câu trả lời của ứng viên</p>
                                            ...
                                            {q.keyPoints?.length > 0 && (
                                                <div className="bg-emerald-500/10 border border-emerald-500/20 text-emerald-300 rounded-lg p-4">
                                                    <p className="text-xs font-bold text-emerald-300 mb-2">✓ Điểm mạnh</p>
                                            ...
                                            {q.missedPoints?.length > 0 && (
                                                <div className="bg-amber-500/10 border border-amber-500/20 text-amber-300 rounded-lg p-4">
                                                    <p className="text-xs font-bold text-amber-300 mb-2">→ Điểm cần cải thiện</p>
                                                    <ul className="space-y-1 text-sm text-white font-medium">
                                            ...
                                            {q.suggestions?.length > 0 && (
                                                <div className="bg-blue-500/10 border border-blue-500/20 text-blue-300 rounded-lg p-4">
                                                    <p className="text-xs font-bold text-blue-300 mb-2">💡 Gợi ý câu trả lời tốt hơn</p>
                                                    <ul className="space-y-1 text-sm text-white font-medium">
                                            ...
                                            {q.aiFeedback && (
                                                <div className="bg-purple-500/10 border border-purple-500/20 text-purple-300 rounded-lg p-4">
                                                    <p className="text-xs font-bold text-purple-300 mb-2">💬 Nhận xét từ AI</p>
                ...
                {session.overallFeedback && (
                    <div className="bg-white/10 border border-white/10 backdrop-blur-md rounded-3xl p-8 shadow-xl">
                        <h3 className="text-xl font-bold text-white mb-4">Đánh giá tổng quan từ AI Mentor</h3>
                        <p className="text-white leading-relaxed font-bold text-base whitespace-pre-line">{session.overallFeedback}</p>
                        {session.nextSteps && session.nextSteps.length > 0 && (
                            <div className="mt-4 border-t border-white/10 pt-4">
                                <p className="text-sm font-bold text-[#F5C518] mb-2">Định hướng tiếp theo:</p>
                                <ul className="space-y-1 text-sm text-white font-medium">
                ...
                <div className="bg-white/10 border border-white/10 backdrop-blur-md rounded-3xl p-8 shadow-xl space-y-6">
                    ...
                    {recMessage && (
                        <div className={`p-5 rounded-2xl border text-sm leading-relaxed ${
                            ...
                            <p className="font-semibold flex items-center gap-2 mb-1">
                                {recPassed ? '💡 AI Nhận xét & Đề xuất:' : '⚠️ Lời khuyên từ AI Mentor:'}
                            </p>
                            <p className="font-medium">{recMessage}</p>
                        </div>
                    )}
                    ...
                    {jobsLoading ? (
                        ...
                        <div className="bg-white/5 rounded-2xl p-6 text-center text-white/50 text-sm">
                            <p>Vui lòng hoàn thành phỏng vấn (trả lời đủ 10 câu hỏi) để AI tiến hành phân tích và gợi ý việc làm.</p>
                        </div>
                    ) : recPassed && recommendedJobs.length > 0 ? (
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            {recommendedJobs.map((job) => (
                                <div
                                    key={job._id}
                                    className="bg-white/5 border border-white/10 hover:border-white/25 rounded-2xl p-4 flex flex-col justify-between hover:bg-white/10 transition-all cursor-pointer group"
                                    onClick={() => {
                                        if (job.externalUrl) {
                                ...
                                >
                                    <div>
                                        <div className="flex items-center justify-between gap-2 mb-2 flex-wrap">
                                            ...
                                            {job.matchScore && (
                                                <span className="text-[10px] bg-[#F5C518]/25 text-[#F5C518] px-2 py-0.5 rounded-full font-bold">
                                                    🔥 AI Khớp: {job.matchScore}%
                                                </span>
                                            )}
                                        </div>
                                        ...
                                        {job.matchReason && (
                                            <div className="mt-3 p-2.5 rounded-xl bg-white/5 border border-white/5 text-[11px] text-white/70 line-clamp-3 hover:line-clamp-none transition-all">
                                                ✨ {job.matchReason}
                                            </div>
                                        )}
                                    </div>
