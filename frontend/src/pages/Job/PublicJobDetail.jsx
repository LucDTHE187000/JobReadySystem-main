...
function RelatedJobCard({ job }) {
    const navigate = useNavigate();
    const recruiter = job.recruiterId || {};
    const company = job.agencyCompanyName || recruiter.companyName || recruiter.name || 'Công ty';
    return (
        <div
            ...
            <div className="relative z-10 flex-1 max-w-6xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-6">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
                    ...
                    <div className="lg:col-span-2 space-y-5">
                        ...
                        {/* Banner link gốc cho công việc thu thập dạng Agency */}
                        {job.externalUrl && (
                            <div className="bg-[#F5C518]/10 border border-[#F5C518]/30 rounded-2xl p-5 shadow-lg flex flex-col sm:flex-row items-center justify-between gap-4">
                                <div className="flex gap-3 items-start min-w-0">
                                    <ExternalLink className="w-5 h-5 text-[#F5C518] flex-shrink-0 mt-0.5" />
                                    <div className="text-left">
                                        <p className="text-sm font-bold text-white">Tin đăng tuyển dụng Agency</p>
                                        <p className="text-xs text-white/70 mt-1 leading-relaxed">
                                            Công việc này được thu thập từ nguồn ngoài <strong>{job.sourcePlatform || 'Nguồn gốc'}</strong>. Bạn có thể bấm để xem và đối chiếu tin đăng gốc trên nền tảng nguồn.
                                        </p>
                                    </div>
                                </div>
                                <a
                                    ...
                                    className="px-4 py-2 bg-[#F5C518] hover:bg-[#D4A800] text-[#0A2463] hover:scale-105 active:scale-95 font-bold rounded-xl text-xs flex items-center gap-1.5 transition-all shadow-md flex-shrink-0"
                                >
                                    Xem tin gốc trên {job.sourcePlatform || 'Platform'} <ArrowRight className="w-3.5 h-3.5" />
                                </a>
                            </div>
                        )}
                                ...
                                <div className="min-w-0">
                                    {recruiter.companyWebsite ? (
                                        <a
                                            ...
                                            target="_blank"
                                            rel="noopener noreferrer"
                                            className="text-sm font-bold text-white leading-snug hover:text-cyan-400 transition-colors"
                                        >
                                            {companyName}
                                        </a>
                                    ) : (
